...
def list_contains_word(list, word):
    for elem in list:
        if word == elem.split('/')[-1]:
            return True
    return False
...
class InpaintingDataset(Dataset):
    def __init__(self, datadir, img_suffix='.jpg', pad_out_to_modulo=None, scale_factor=None):
        ...
        if len(self.mask_filenames) != len(self.img_filenames):
            self.img_filenames = [
                x for x in self.img_filenames if list_contains_word(self.mask_filenames, x.split('/')[-1])
            ]
            self.mask_filenames = [
                x for x in self.mask_filenames if list_contains_word(self.img_filenames, x.split('/')[-1])
            ]
