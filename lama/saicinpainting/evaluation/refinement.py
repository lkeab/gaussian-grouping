...
def _infer(
    ...
    mask[0][0] = torch.from_numpy(tmp)

    mask = torch.from_numpy(cv2.resize(mask.cpu().detach().numpy()[0][0],
                                       (image.shape[3], image.shape[2]),
                                       interpolation=cv2.INTER_NEAREST)).to(image.device)[None, None, ...].repeat_interleave(3, dim=1)
...
def refine_predict(
    ...
    gpu_ids = [f'cuda:{gpuid}' for gpuid in gpu_ids.replace(" ", "").split(",") if gpuid.isdigit()][:1]
    # print(gpu_ids)
    n_resnet_blocks = 0
